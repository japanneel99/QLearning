...
class QtableAgent:
    def __init__(
        ...
        # Parameters needed for Q learning
    ...
    def experience(self, states, actions, terminal, reward):
        for s, a, t, r in zip(states, actions, terminal, reward):
            self.states = s
            # find nearest action in action_candidates
            self.last_action_index = np.argmin(
                np.abs(self.action_candidates - a))
            self.observe(r, t)
    ...
    def observe(self, reward, terminal):
        # update q table with self.index and self.last_action_index
        ...
        # self.__set_q_value(
        #     self.states, self.last_action_index,
        #     self.__get_q_value(self.states, self.last_action_index) + (self.alpha*td_delta))

        q_value, discrete_states = self.__set_q_value(
            self.states, self.last_action_index,
            self.__get_q_value(self.states, self.last_action_index) + (self.alpha*td_delta))
    ...
    def save(self, directory='saved_variables'):
        ...
        args = (self.action_candidates, self.quantization, self.epsilon,
